...
parser.add_argument("--dataset_names", default=['WP_IRSTD_9000'], nargs='+',
                    help="dataset_name: 'NUAA-SIRST', 'NUDT-SIRST', 'IRSTD-1K', 'NUDT-SIRST-Sea', 'SIRST3','ICPR_point'")
...
parser.add_argument("--batchSize", type=int, default=16, help="Training batch sizse, default: 16") # 16
parser.add_argument("--patchSize", type=int, default=256, help="Training patch size, default: 256")
parser.add_argument("--patchSize_test_crop", type=int, default=1536, help="Training patch size, default: 256")
...
parser.add_argument("--nEpochs", type=int, default=300, help="Number of epochs, default: 400")
...
parser.add_argument("--cache", default=False, type=str, help="True: cache intermediate masks results, False: save intermediate masks results")
...
def update_gt_mask(save_pth, thresh_Tb, thresh_k):
    ...
    with torch.no_grad():
        for idx_iter, (img, gt_mask, gt_mask_update, update_dir, size) in tqdm(enumerate(update_loader)):
            ...
            b, c, h, w = img.shape
            if h > opt.patchSize_test_crop and w > opt.patchSize_test_crop:
                img_unfold = F.unfold(img[:, :, :, :], opt.patchSize_test_crop, stride=opt.patchSize_test_crop)
                img_unfold = img_unfold.reshape(c, opt.patchSize_test_crop, opt.patchSize_test_crop, -1).permute(3, 0, 1, 2)
                ...
                pred = F.fold(preds.reshape(1, -1, patch_num), kernel_size=opt.patchSize_test_crop, stride=opt.patchSize_test_crop,
                              output_size=(h, w))
            else:
                pred = net.forward(img)
    ...
    opt.f.write("Evolution masks pixAcc, mIoU:\t" + str(results1) + '\n')
    print("Evolution masks pixAcc, mIoU:\t" + str(results1))
...
def test(save_pth):
    ...
    with torch.no_grad():  # 禁用梯度计算，减少内存使用
        for idx_iter, (img, gt_mask, size, _) in enumerate(test_loader):
            ...
            if h > opt.patchSize_test_crop and w > opt.patchSize_test_crop:
                img_unfold = F.unfold(img[:, :, :, :], opt.patchSize_test_crop, stride=opt.patchSize_test_crop)
                img_unfold = img_unfold.reshape(c, opt.patchSize_test_crop, opt.patchSize_test_crop, -1).permute(3, 0,
                                                                                                                 1, 2)
                patch_num = img_unfold.size(0)
                for pi in range(patch_num):
                    img_pi = img_unfold[pi, :, :, :].unsqueeze(0).float()
                    ...
                    if pi == 0:
                        ...
                        preds = torch.cat([preds, preds_pi], dim=0)
                preds = preds.permute(1, 2, 3, 0).unsqueeze(0)
                pred = F.fold(preds.reshape(1, -1, patch_num), kernel_size=opt.patchSize_test_crop,
                              stride=opt.patchSize_test_crop,
                              output_size=(h, w))
            else:
                pred = net.forward(img)
            pred = pred[:,:,:size[0],:size[1]]
    ...
    print("Inference masks pixAcc, mIoU:\t" + str(results1))
    print("Inference masks PD, FA:\t" + str(results2))
...
if __name__ == '__main__':
    ...
    for dataset_name in opt.dataset_names:
        ...
        for model_name in opt.model_names:
            ...
            if opt.cache:
                ### cache intermediate masks results
               with open(opt.dataset_dir+'/img_idx/train_' + opt.dataset_name + '.txt', 'r') as f:
                ...
                ### save intermediate masks results to
                opt.masks_update = opt.dataset_dir + '/' + opt.dataset_name + '_' + opt.save_perdix + '_' + (time.ctime()).replace(' ', '_')
                opt.masks_update = sanitize_path(opt.masks_update)
                # print(opt.masks_update)
